...
namespace _10_Search_Y_Share_Contract_DESDE_CERO.Contracts
{
    ...
    /// </summary>
    public sealed partial class SharePage
    {

        public SharePage()
        {
            this.InitializeComponent();
            Modelo = new ShareModel();
            this.DataContext = Modelo;
        }

        public ShareModel Modelo { get; set; }
        ...
        protected async override void OnNavigatedTo(NavigationEventArgs e)
        {
            ...
            if (args != null)
            {
                if (args.ShareOperation.Data.Contains("Bitmap"))
                {
                    Modelo.ImageTitle = "Bitmap: " + args.ShareOperation.Data.Properties.Title;
                    var bmpRef = await args.ShareOperation.Data.GetBitmapAsync();
                    await Modelo.ImageBitmap.SetSourceAsync(await bmpRef.OpenReadAsync());
                }
                else
                {
                    Modelo.ImageTitle = "Attached: " + args.ShareOperation.Data.Properties.Title;
                    var storageItems = await args.ShareOperation.Data.GetStorageItemsAsync();
                    var theOne = storageItems.FirstOrDefault();

                    if (theOne != null)
                    {
                        var storageFile = theOne as StorageFile;
                        if (storageFile != null)
                        {
                            var rars = RandomAccessStreamReference.CreateFromFile(storageFile);
                            await Modelo.ImageBitmap.SetSourceAsync(await rars.OpenReadAsync());
                        }
                    }
                }
            }
